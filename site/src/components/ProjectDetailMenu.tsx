...
export function ProjectDetailMenu({
  ...
  const buttonBase =
    buttonVariant === 'plain'
      ? `relative z-30 inline-flex h-12 w-12 items-center justify-center rounded-xl border border-transparent bg-transparent p-0 font-display text-sm font-semibold tracking-[0.24em] transition-all duration-300 ${
          isOpen
  ...
  const columns: Array<{ key: ProjectDetailTabKey; label: string }[]> = [];
  for (let i = 0; i < tabs.length; i += COLUMN_SIZE) {
    columns.push(tabs.slice(i, i + COLUMN_SIZE));
  }
  ...
  return (
    <>
      ...
      <div
        id={menuId}
        className={`absolute ${align === 'right' ? 'right-0' : 'left-0'} top-[calc(100%+12px)] z-50 isolate ${isMultiColumn ? 'w-[392px]' : 'w-[190px]'} overflow-hidden rounded-2xl border border-neon-cyan/58 bg-[#06192e]/88 p-2.5 shadow-[0_24px_70px_rgba(0,0,0,0.76),0_0_30px_rgba(0,245,255,0.2),inset_0_1px_0_rgba(255,255,255,0.14),inset_0_0_38px_rgba(0,245,255,0.08)] backdrop-blur-[22px] backdrop-saturate-150 transition-all duration-300 ${
          isOpen
      ...
      >
        ...
        <span
          ...
          aria-hidden
        />
        <div className="relative flex flex-row-reverse items-start gap-1.5">
