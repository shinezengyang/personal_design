...
import { QingyuFigmaCase } from '../components/QingyuFigmaCases';
import { ResponsiveScaleFrame } from '../components/ResponsiveScaleFrame';
import {
  HighSeasCleanupGangExactCase,
  HighSeasNavyTrialExactCase,
  renderStarFigmaCase,
} from '../components/StarFigmaCases';
...
const QINGYU_PERSONAS: { title: string; ratio: string; icon: LucideIcon; need: string; design: string }[] = [
  { title: '沉浸型', ratio: '43%', icon: BookOpen, need: '剧情不被打断，UI 不遮挡叙事', design: '演出模式极简 HUD' },
  { title: '成就型', ratio: '35%', icon: Swords, need: '养成路径短，数值反馈即时', design: '一键直达，进度实时可视' },
  { title: '社交型', ratio: '22%', icon: Users, need: '社交入口随时可达，不干扰操作', design: '快捷入口常驻，分层不遮挡' },
];
    ...
    <div className="grid gap-4 md:grid-cols-3">
      {QINGYU_PERSONAS.map((p) => {
        ...
        return (
          ...
          >
            <div className="flex flex-col items-center gap-3 border-b border-[#87C8BC]/10 px-5 py-5 text-center">
              <div className="font-mono text-3xl font-bold leading-none text-neon-cyan">{p.ratio}</div>
...
function QingyuMainScreenDiagram() {
  ...
  const pcCallouts: HudCallout[] = [
    ...
    { n: '5', label: '聊天栏区域', side: 'left' as const, y: 555, targetX: 305 },
    { n: '6', label: '跑马灯', side: 'right' as const, y: 21, targetX: 1152 },
  ...
  const pcNotes = [
    ...
    ['5', '聊天栏区域', '常驻显示帮会、队伍等频道消息，支持展开浏览与快捷输入。'],
    ['6', '跑马灯', '用于滚动展示系统公告、活动信息等需要持续曝光的内容。'],
    ...
    ['11', '副技能栏区域', '横向排布可绑定键位的技能槽，与键盘快捷键、手柄按键一一对应。'],
...
export default function ProjectDetail({
  ...
  useLayoutEffect(() => {
    ...
    const backBtn = root.querySelector<HTMLElement>('.project-detail-header .cyber-btn');
    const menuBtn = root.querySelector<HTMLElement>('.project-detail-header .relative');
    const heroCard =
      cardRef.current && !cardRef.current.classList.contains('hidden') ? cardRef.current : null;
    const heroImgWrap = heroCard?.querySelector<HTMLElement>('.gradient-border') ?? null;
    const heroImg = heroCard?.querySelector<HTMLElement>('img') ?? null;
    const heroCategoryLine = heroCard?.querySelector<HTMLElement>('.font-mono.text-sm') ?? null;
    const heroTitle = heroCard?.querySelector<HTMLElement>('h1') ?? null;
    const heroDesc = heroCard?.querySelector<HTMLElement>('p.text-cyber-gray') ?? null;
    const heroTags = heroCard
    ...
    const everything: Element[] = [
      ...
      ...(heroImg ? [heroImg] : []),
      ...(heroCategoryLine ? [heroCategoryLine] : []),
      ...(heroTitle ? [heroTitle] : []),
      ...(heroDesc ? [heroDesc] : []),
      ...heroTags,
      ...modules,
    ...
    // ── Above-the-fold choreography ──
    // Back button slides from left, menu button from right
    if (backBtn) gsap.set(backBtn, { opacity: 0, x: -30 });
    if (menuBtn) gsap.set(menuBtn, { opacity: 0, x: 30 });
    // Hero card container fades up
    if (heroCard) gsap.set(heroCard, { opacity: 0, y: 32 });
    // Image wrapper clips in with scale
    if (heroImgWrap) gsap.set(heroImgWrap, { opacity: 0, scale: 0.92 });
    if (heroImg) gsap.set(heroImg, { opacity: 0, scale: 1.08 });
    // Text elements slide up independently
    if (heroCategoryLine) gsap.set(heroCategoryLine, { opacity: 0, x: -20 });
    if (heroTitle) gsap.set(heroTitle, { opacity: 0, y: 20, scale: 0.97 });
    if (heroDesc) gsap.set(heroDesc, { opacity: 0, y: 16 });
    ...
    // t=0: buttons fly in from sides
    if (backBtn) leadIn.to(backBtn, { opacity: 1, x: 0, duration: 0.5, clearProps: 'transform' }, 0);
    if (menuBtn) leadIn.to(menuBtn, { opacity: 1, x: 0, duration: 0.5, clearProps: 'transform' }, 0.06);

    // t=0.12: hero card frame rises
    if (heroCard) leadIn.to(heroCard, { opacity: 1, y: 0, duration: 0.65, clearProps: 'transform' }, 0.12);

    // t=0.3: image wrapper scales up, inner image counter-scales for cinematic zoom-settle
    if (heroImgWrap) leadIn.to(heroImgWrap, { opacity: 1, scale: 1, duration: 0.75, ease: 'power2.out', clearProps: 'transform' }, 0.3);
    if (heroImg) leadIn.to(heroImg, { opacity: 1, scale: 1, duration: 0.9, ease: 'power2.out', clearProps: 'transform' }, 0.35);

    // t=0.55: category slides in from left
    if (heroCategoryLine) leadIn.to(heroCategoryLine, { opacity: 1, x: 0, duration: 0.45, clearProps: 'transform' }, 0.55);

    // t=0.62: title scales up
    if (heroTitle) leadIn.to(heroTitle, { opacity: 1, y: 0, scale: 1, duration: 0.55, ease: 'back.out(1.4)', clearProps: 'transform' }, 0.62);

    // t=0.75: description fades in
    if (heroDesc) leadIn.to(heroDesc, { opacity: 1, y: 0, duration: 0.5, clearProps: 'transform' }, 0.75);
    ...
    const observer = new IntersectionObserver(
      (entries, obs) => {
        entries.forEach((entry) => {
          ...
          const titles = Array.from(el.querySelectorAll<HTMLElement>('.neon-text-pink, .neon-text-cyan')).filter(
            (t) => t.closest('.project-detail-module') === el,
          );
          const subtitles = Array.from(el.querySelectorAll<HTMLElement>('.font-display.text-base')).filter(
            (t) => t.closest('.project-detail-module') === el,
          );
          const cards = Array.from(el.querySelectorAll<HTMLElement>('.project-detail-inner-card'));
          const svgs = Array.from(el.querySelectorAll<HTMLElement>('svg[role="img"]'));

          const allInner = [...titles, ...subtitles, ...cards, ...svgs];
          if (titles.length) gsap.set(titles, { opacity: 0, x: -24 });
          if (subtitles.length) gsap.set(subtitles, { opacity: 0, x: -16 });
          if (cards.length) gsap.set(cards, { opacity: 0, y: 22, scale: 0.97 });
          if (svgs.length) gsap.set(svgs, { opacity: 0, y: 18, scale: 0.98 });

          const tl = gsap.timeline();

          // Module container fades in
          tl.to(el, { opacity: 1, y: 0, duration: 0.55, ease: 'power3.out', clearProps: 'opacity,transform' }, 0);

          // Section titles slide from left
          if (titles.length)
            tl.to(titles, { opacity: 1, x: 0, duration: 0.5, stagger: 0.06, ease: 'power3.out', clearProps: 'transform' }, 0.1);

          // Subtitles follow titles
          if (subtitles.length)
            tl.to(subtitles, { opacity: 1, x: 0, duration: 0.4, stagger: 0.05, ease: 'power3.out', clearProps: 'transform' }, 0.2);

          // SVG diagrams scale in
          if (svgs.length)
            tl.to(svgs, { opacity: 1, y: 0, scale: 1, duration: 0.6, stagger: 0.1, ease: 'power2.out', clearProps: 'transform' }, 0.25);

          // Inner cards stagger in with slight bounce
          if (cards.length)
            tl.to(cards, { opacity: 1, y: 0, scale: 1, duration: 0.5, stagger: 0.07, ease: 'back.out(1.2)', clearProps: 'transform' }, 0.3);

          // Clean up any leftover inner element styles after timeline completes
          tl.call(() => { gsap.set(allInner, { clearProps: 'all' }); });
  ...
  const activeDetailTabLabel =
    detailTabs.find((tab) => tab.key === resolvedActiveDetailTab)?.label ?? project.title;
  const starFigmaDetailCase = renderStarFigmaCase(project.title);
  const isStarStandaloneFigmaCase = Boolean(starFigmaDetailCase) && resolvedActiveDetailTab === 'framework';
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div ref={cardRef} className={`project-detail-hero glass-card rounded-2xl p-6 sm:p-10 card-glow${(usesHighSeasDetailLayout && resolvedActiveDetailTab !== 'framework') || isQingyuCaseTab || isStarStandaloneFigmaCase ? ' hidden' : ''}`}>
            ...
            <>
              ...
              <div className="project-detail-module glass-card rounded-2xl p-6 sm:p-8 card-glow">
                ...
                <div className="space-y-6">
                  ...
                  <QingyuUICard
                    title="三级界面（横版）"
                    img="qingyu-level3-horizontal-ui.webp"
                    callouts={[
                  ...
                  <QingyuUICard
                    title="三级界面（竖版）"
                    img="qingyu-level3-vertical-ui.webp"
                    callouts={[
              ...
              {usesHighSeasDetailLayout ? (
                <ResponsiveScaleFrame minDesignWidth={900} maxScale={1.25} className="mt-6">
                  <div className="grid w-[900px] grid-cols-3 gap-6">
                    {(
                      [
                        { src: publicUrl('high-seas-persona-1.webp'), alt: 'High Seas Hero 玩家画像 1：休闲' },
                        { src: publicUrl('high-seas-persona-2.webp'), alt: 'High Seas Hero 玩家画像 2：乐趣' },
                        { src: publicUrl('high-seas-persona-3.webp'), alt: 'High Seas Hero 玩家画像 3：消费' },
                      ] as const
                    ).map((item) => (
                      <div
                        key={item.src}
                    ...
                    {pencilNode === '8qvkJ' ? (
                      <div className="mt-6 space-y-6 pb-8">
                        <StructureLayerPlaceholderCard
                          title="半屏界面"
                          imageSrc="pencil/xingji-aodaisai/half-screen.webp"
                          imageAlt="半屏界面示意图"
                        ...
                        <StructureLayerPlaceholderCard
                          title="弹窗界面"
                          imageSrc="pencil/xingji-aodaisai/modal-screen.webp"
                          imageAlt="弹窗界面示意图"
